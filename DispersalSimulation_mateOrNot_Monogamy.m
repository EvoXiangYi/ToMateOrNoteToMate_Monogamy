function []= DispersalSimulation_mateOrNot_Monogamy(InInitialSexProb, InTimeSteps, InNumGrid, InPopSize, InNumHabitats, InpSpatial, InpTemporal, InfitnessIOS, InMeanDispersalTrait_init, InSigmaDispersalTrait_init, InSigmaTraitCoordinates, InSigmaDispersal, InDispersalDeathRate, InNumRepeats)
	
	%%%%%%%%%%%%%%%%%%%%% start Parameters Initialization  %%%%%%%%%%%%%%%%%%%%%%%%%
	initSexProb = InInitialSexProb;
	...
	population(probMating,:) = initSexProb;
	...
	for t=1:timeSteps
		...
		for i=1:totalEffectiveHabitats
			newWives=getMarried(femalesOnHabitatsCell{i}, femalesFitnessOnHabitatsCell{i}, malesOnHabitatsCell{i}, malesFitnessOnHabitatsCell{i});
		...
		for i=1:popSize 
			...
			chosenMom = myID_parent([MatrixWife(2,:), asexual_females_fitness]);
			MomID = [MatrixWife(1,:),asexual_females](chosenMom);
			momGenome=population(:,MomID);
			if ismember(MomID,asexual_females)
				young(:,i) = momGenome;
			else
				MomIdxInMatrixWife=find(MatrixWife(1,:)==MomID);
				DadID = MatrixWife(3,MomIdxInMatrixWife);
	...
	csvwrite([strcat("ToMateOrNot-Monogamy-InitDispProb-", num2str(init_Prob_dispersal), "-numGrid-", num2str(numGrid), "-numHabitats-", num2str(numHabitats),"-Rep-",num2str(numRepeats),"-InitSex-", num2str(initSexProb), ".csv")], SummaryData);
end